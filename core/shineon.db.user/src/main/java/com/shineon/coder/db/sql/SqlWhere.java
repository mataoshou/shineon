package com.shineon.coder.db.sql;
...
public class SqlWhere
{
	...
	{
		 add( DBSql.name(name) + op + DBSql.value(value.toString()));
	}
	...
	public void addIn(String name, List values)
	{
		...
		String sql =  DBSql.name(name) + " IN (";
		
		for(int i=0; i<values.size(); i++)
		{
			String s = DBSql.value( values.get(i).toString());
			sql += s;
